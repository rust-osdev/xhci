...
macro_rules! wo_bit_cx {
    ...
    ($bit:literal,$method:ident,$name:literal) => {
        bit_modifier_cx!($bit, $method, $name);
    };
}
...
macro_rules! rw1s_bit_cx {
    ([$offset:literal]($bit:literal),$method:ident,$name:literal) => {
        bit_getter_cx!([$offset]($bit), $method, $name);
        w1s_bit_cx!([$offset]($bit), $method, $name);
    };
    ($bit:literal,$method:ident,$name:literal) => {
        bit_getter_cx!($bit, $method, $name);
        w1s_bit_cx!($bit, $method, $name);
    };
}
...
macro_rules! ro_field_cx {
    ...
    ($range:expr,$method:ident,$name:literal,$ty:ty) => {
        field_getter_cx!($range, $method, $name, $ty);
    };
}
...
macro_rules! rw_field_cx {
    ...
    ($range:expr,$method:ident,$name:literal,$ty:ty) => {
        field_getter_cx!($range, $method, $name, $ty);
        field_setter_cx!($range, $method, $name, $ty);
    };
}
